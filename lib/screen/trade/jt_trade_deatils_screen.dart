...
import 'package:classia_amc/service/apiservice/wallet_service.dart';
import 'package:classia_amc/service/apiservice/user_service.dart';
import 'package:classia_amc/service/apiservice/jt_trade_service.dart';
import 'package:classia_amc/utills/constent/user_constant.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'dart:ui';
...
class _TradingDetailsScreenState extends State<JtTradeDeatilsScreen>
  ...
  late WalletService _walletService;
  late UserService _userService;
  late JtTradeService _tradeService;
  late AnimationController _animationController;
  late Animation<double> _fadeAnimation;
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: _buildMetricCard('AUM', '₹5,000 Cr', Icons.account_balance_wallet)),
                    SizedBox(width: 8.w),
                    Expanded(child: _buildMetricCard('1Y Return', widget.fundData['oneYearChange'] ?? '12.5%', Icons.trending_up)),
                    SizedBox(width: 8.w),
                    Expanded(child: _buildMetricCard('Jockey Point', '${widget.value}%', FontAwesomeIcons.star)),
                  ],
                ),
                ...
                children: [
                  TextFormField(
                    ...
                    validator: (value) {
                      ...
                      if (amount == null || amount <= 0) return 'Enter a valid amount';
                      if (amount < 1000) return 'Minimum investment amount is ₹1,000';
                      return null;
                    },
                    onChanged: (value) => setState(() {}),
                  ),
  ...
  Future<void> _handleInvestOrWithdraw(String action) async {
    ...
    try {
      if (action == 'Invest') {
        ...
        final response = await _tradeService.purchaseLumpsum(
          totAmt: amount,
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            SizedBox(height: 8.h),
            Text(
              'Your $action of ₹${_amountController.text} has been initiated.',
              style: TextStyle(color: AppColors.secondaryText, fontSize: 14.sp, height: 1.4),
              textAlign: TextAlign.center,
            ),
            SizedBox(height: 16.h),
