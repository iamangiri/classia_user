...
try {
final response = await _walletService.getTransactionList(_page, _limit, transactionType: 'WITHDRAW');
setState(() {
transactions.addAll(List<Map<String, dynamic>>.from(response['transactions']));
...
final date = DateTime.parse(transaction['CreatedAt']).toString().split('.')[0];
